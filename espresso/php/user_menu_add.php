...
while($row = mysqli_fetch_assoc($all_session1)){   
    ...
    for ($i=0; $i < $numbeOfItems; $i++) {
        if(isset($_POST['add_to_cart'. $i])){
            ...
            $insert_product = mysqli_query($dbcon, "UPDATE carts SET prod_name='$product_name', prod_price='$tp', prod_image='$product_image', quantity='$total' WHERE prod_name='$product_name'");
